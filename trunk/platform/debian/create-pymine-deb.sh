...
Description: The Mine Project: Pymine Development Version
 ...
 that enables personal data sharing via feeds and URLs which are
 different for each subscriber (unlike a traditional blog)
EOF
...
( cd debian/$PYPARENT/pymine ; rm -r `find . -name .svn` )

dpkg-deb --build debian || exit 1

DATESTAMP=`date "+%Y%m%d%H%M%S"`

mv debian.deb /tmp/pymine_${PYMINE_VERSION}_${DATESTAMP}.deb
